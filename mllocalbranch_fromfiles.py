...
class RegressionInitialK_KPrime(MlLocalbranch):
    ...
    def generate_regression_samples_k_prime(self, t_limit, instance_size='-small'):
        ...
        self.k_samples_directory = self.directory + 'k_samples_k_prime' + '/'
        self.regression_samples_directory = self.directory + 'regression_samples_k_prime' + '/'
        pathlib.Path(self.regression_samples_directory).mkdir(parents=True, exist_ok=True)
        ...
        while index_instance < 100:
            ...
            data_sample = [observation, k_init]
            saved_name = f'{self.instance_type}-{str(index_instance)}_transformed'
            filename = f'{self.regression_samples_directory}regression-{saved_name}.pkl'
            with gzip.open(filename, 'wb') as f:
                pickle.dump(data_sample, f)
    ...
    def execute_regression_mergedatasets(self, lr=0.0000001, n_epochs=20):
        ...
        # load the small dataset
        small_dataset =  "setcover-independentset-combinatorialauction-generalizedis"
        ...
        train_data = torch.utils.data.ConcatDataset([train_dataset_0, train_dataset_1, train_dataset_2, train_dataset_3, train_dataset_4, train_dataset_5, train_dataset_6, train_dataset_7]) #, , train_dataset_3
        valid_data = torch.utils.data.ConcatDataset([valid_dataset_0, valid_dataset_1, valid_dataset_2, valid_dataset_3, valid_dataset_4, valid_dataset_5, valid_dataset_6, valid_dataset_7]) # valid_dataset_1, valid_dataset_3
        test_data = torch.utils.data.ConcatDataset([test_dataset_0, test_dataset_1,  test_dataset_2, test_dataset_3, test_dataset_4, test_dataset_5, test_dataset_6, test_dataset_7]) #test_dataset_1, test_dataset_3
    ...
    def evaluate_localbranching_k_prime(self, test_instance_size='-small', train_instance_size='-small', total_time_limit=60,
        ...
        index_instance = 100
        # if test_instance_size == '-large':
        #     index_instance = 145

        while index_instance < 115:
    ...
    def primal_integral(self, test_instance_size, total_time_limit=60, node_time_limit=30):
        ...
        # if not (i == 161 or i == 170):
            ...
            a = [objs.min(), objs_pred.min(), objs_pred_reset.min(), objs_2.min(), objs_pred_2.min(), objs_pred_reset_2.min(), objs_k_prime.min(), objs_k_prime_2.min()]  # objs_2.min(), objs_pred_2.min(), objs_pred_reset_2.min()
            # a = [objs.min(), objs_pred.min(), objs_pred_reset.min()]
            obj_opt = np.amin(a)
        ...
        primal_gap_final_preds = primal_gap_final_preds.sum() / len(primal_gap_final_preds)
        primal_gap_final_preds_reset = primal_gap_final_preds_reset.sum() / len(primal_gap_final_preds_reset)
        primal_gap_final_regression_k_primes = primal_gap_final_regression_k_primes.sum() / len(primal_gap_final_regression_k_primes)
        ...
        print('k_pred primal gap: ', primal_gap_final_preds)
        print('k_pred_reset primal gap: ', primal_gap_final_preds_reset)
        print('k_regre_prime primal gap: ', primal_gap_final_regression_k_primes)
        ...
        ax.plot(t, primalgap_regression_k_prime_ave, label='lb-regression-k-prime')
        ax.set_xlabel('time /s')
        ax.set_ylabel("normalized primal gap")
        ax.legend()
        plt.show()

    def primal_integral_k_prime(self, test_instance_size, total_time_limit=60, node_time_limit=30):
        ...
        # if not (i == 161 or i == 170):
            ...
            with gzip.open(filename, 'rb') as f:
                data = pickle.load(f)
            objs, times, objs_k_prime, times_k_prime = data  # objs contains objs of a single instance of a lb test

            # filename = f'{directory_lb_test_k_prime_2}lb-test-{instance_name}.pkl'
            #
            # with gzip.open(filename, 'rb') as f:
            #     data = pickle.load(f)
            # objs_2, times_2, objs_k_prime_2, times_k_prime_2 = data  # objs contains objs of a single instance of a lb test

            a = [objs.min(), objs_k_prime.min()]  # objs_2.min(), objs_pred_2.min(), objs_pred_reset_2.min()
            # a = [objs.min(), objs_pred.min(), objs_pred_reset.min()]
            obj_opt = np.amin(a)
...
class RlLocalbranch(MlLocalbranch):
    ...
    def load_mip_dataset(self, instances_directory=None, sols_directory=None, incumbent_mode=None):
        ...
        instance_files = [str(path) for path in sorted(pathlib.Path(instances_directory).glob(instance_filename), key=lambda path: int(path.stem.replace('-', '_').rsplit("_", 2)[1]))]

        instance_train_files = instance_files[:int(0.05 * len(instance_files))]
        instance_valid_files = instance_files[int(0.05 * len(instance_files)):int(0.5 * len(instance_files))]
        instance_test_files = instance_files[int(0.5 * len(instance_files)):]

        sol_files = [str(path) for path in sorted(pathlib.Path(sols_directory).glob(sol_filename), key=lambda path: int(path.stem.replace('-', '_').rsplit("_", 2)[1]))]
        sol_train_files = sol_files[:int(0.05 * len(sol_files))]
        sol_valid_files = sol_files[int(0.05 * len(sol_files)):int(0.5 * len(sol_files))]
        sol_test_files = sol_files[int(0.5 * len(sol_files)):]
    ...
    def train_agent_per_instance(self, MIP_model, incumbent_solution, node_time_limit, total_time_limit, index_instance,
        ...
        # regression_model_gnn.load_state_dict(torch.load(
        #      'trained_params_' + self.instance_type + '.pth'))
    ...
    def train_agent(self, train_instance_size='-small', total_time_limit=60, node_time_limit=10,
        ...
        self.reinforce_train_directory = train_directory + 'rl/' + 'reinforce/train/noimitation/'
    ...
    def evaluate_lb_per_instance_rlactive(self, node_time_limit, total_time_limit, index_instance, reset_k_at_2nditeration=False,
        ...
        status, obj_best, elapsed_time, lb_bits_pred_reset, times_regression_reinforce, objs_regression_reinforce, agent1 = self.mdp_localbranch(
            localbranch=lb_model3,
            is_symmetric=self.is_symmetric,
            reset_k_at_2nditeration=reset_k_at_2nditeration,
            agent=agent1,
        ...
        status, obj_best, elapsed_time, lb_bits_pred, times_noregression_reinforce, objs_noregression_reinforce, agent2 = self.mdp_localbranch(
    ...
    def evaluate_localbranching_rlactive(self, evaluation_instance_size='-small', total_time_limit=60, node_time_limit=30,
        ...
        checkpoint = torch.load(
            # self.saved_gnn_directory + 'checkpoint_simplepolicy_rl4lb_reinforce_lr' + str(lr) + '_epsilon' + str(epsilon) + '.pth'
            self.saved_rlmodels_directory + 'checkpoint_noregression_noimitation_reward3_simplepolicy_rl4lb_reinforce_lr0.01_epsilon0.0_epoch210.pth'
        )
        ...
        index_instance = 100
        while index_instance < 200:
            index_instance, agent1, agent2 = self.evaluate_lb_per_instance_rlactive(node_time_limit=node_time_limit,
                                                           total_time_limit=total_time_limit,
                                                           index_instance=index_instance,
                                                           reset_k_at_2nditeration=reset_k_at_2nditeration,
                                                           agent1=agent1,
    ...
    def primal_integral(self, test_instance_size, total_time_limit=60, node_time_limit=30):
        ...
        for i in range(100,200):
            ...
            with gzip.open(filename, 'rb') as f:
                data = pickle.load(f)
            objs_reinforce_talored, times_reinforce_talored, objs_regresison_reinforce_talored, times_regression_reinforce_talored = data  # objs contains objs of a single instance of a lb test
            ...
            a = [objs_regression.min(),objs_reinforce.min(), objs_regresison_reinforce.min(), objs.min(), objs_reinforce_talored.min(), objs_regresison_reinforce_talored.min()]
            obj_opt = np.amin(a)
            ...
            primal_int_regression, primal_gap_final_regression, stepline_regression = self.compute_primal_integral(
                times=times_regression, objs=objs_regression, obj_opt=obj_opt, total_time_limit=total_time_limit)
        ...
        ax.plot(t, primalgap_baseline_ave, label='lb-baseline', color='tab:blue')
        ax.plot(t, primalgap_regression_ave, label='lb-regression', color ='tab:orange')
